...
public class VMReporteEstudiantesPreInscritosProyecto {
	...
	public void llenarListaDinamica() {
		...
		/**
		 * Guarda la Selección en las variables fechaInicio y fechaFin
		 */
	...
	/**				
	 * Método que Carga en el combo de proyecto 
	...
	public void seleccionFiltro() {
		if (seleccion == "Nombre") {
			...
			if (seleccion == "Cédula") {
				txtFiltroEstudiantesPreInscritos.setPlaceholder("Cédula");
				popOpciones.close();
			} else {
				if (seleccion == "Apellido")
	...
	/**
	 * Métodos set y get que permite manipular la vista .zul
	 **/
